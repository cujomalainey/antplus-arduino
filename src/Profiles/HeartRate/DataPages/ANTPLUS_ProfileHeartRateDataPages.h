#ifndef ANTPLUS_PROFILEHEARTRATEDATAPAGES_h
#define ANTPLUS_PROFILEHEARTRATEDATAPAGES_h

/* RX */
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateBatteryStatus.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateCapabilities.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateCumulativeOperatingTime.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateManufacturerInformation.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateModeSettings.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRatePreviousHeartBeat.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateProductInformation.h>
#include <Profiles/HeartRate/DataPages/RX/ANTPLUS_HeartRateSwimIntervalSummary.h>

/* TX */
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateBatteryStatusMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateCapabilitiesMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateCumulativeOperatingTimeMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateManufacturerInformationMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateModeSettingsMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRatePreviousHeartBeatMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateProductInformationMsg.h>
#include <Profiles/HeartRate/DataPages/TX/ANTPLUS_HeartRateSwimIntervalSummaryMsg.h>

#endif // ANTPLUS_PROFILEHEARTRATEDATAPAGES_h